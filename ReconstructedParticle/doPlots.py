...
def drawStack(name, ylabel, legend, leftText, rightText, formats, directory, logY, stacksig, histos, colors, ana_tex, extralab, scaleSig, customLabel, nsig, nbkg, legend2=None, yields=None, plotStatUnc=False, xmin=-1, xmax=-1, ymin=-1, ymax=-1, xtitle="", ytitle=""):
    ...
    if 'AAAyields' in name:
        ...
        text = '#bf{#it{' + 'Yields' +'}}'
        Text.SetTextSize(0.03)
        Text.DrawLatex(0.35, 0.55, text)

        text = '#bf{#it{' + 'Raw MC' +'}}'
        Text.SetTextSize(0.03)
        Text.DrawLatex(0.55, 0.55, text)

        text = '#bf{#it{' + 'Stat. Err.' +'}}'
        Text.SetTextSize(0.03)
        Text.DrawLatex(0.75, 0.55, text)


        for y in yields:
            text = '#bf{#it{' + yields[y][0] +'}}'
            Text.SetTextSize(0.03)
            Text.DrawLatex(0.18, 0.5-dy*0.05, text)

            stry=str(round(yields[y][1],6))
            #stry=stry.split('.')[0]
            text = '#bf{#it{' + stry +'}}'
            Text.SetTextSize(0.03)
            Text.DrawLatex(0.35, 0.5-dy*0.05, text)

            stry=str(yields[y][2])
            stry=stry.split('.')[0]
            text = '#bf{#it{' + stry +'}}'
            Text.SetTextSize(0.03)
            Text.DrawLatex(0.55, 0.5-dy*0.05, text)

            stry=str(round(np.sqrt(yields[y][2])*yields[y][1]/yields[y][2], 6))
            #stry=stry.split('.')[0]
            text = '#bf{#it{' + stry +'}}'
            Text.SetTextSize(0.03)
            Text.DrawLatex(0.75, 0.5-dy*0.05, text)
